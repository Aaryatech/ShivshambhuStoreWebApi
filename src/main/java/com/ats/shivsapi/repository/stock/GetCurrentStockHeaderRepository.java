...
public interface GetCurrentStockHeaderRepository extends JpaRepository<GetCurrentStock, Integer>{
	...
	@Query(value=("SELECT "
			...
			+ "0) AS issue_landing_value, coalesce((Select SUM((po_detail.landing_cost/po_detail.item_qty)*item_issue_detail.item_issue_qty) "
			...
			+ "and po_detail.po_detail_id=t_mrn_detail.po_detail_id AND item_issue_detail.status = 2), 0) AS issue_qty_value, coalesce(0)  AS return_issue_qty, coalesce((Select "
	...
	@Query(value=("SELECT "
			...
			+ "coalesce((Select SUM(t_damage.qty*t_damage.value) FROM t_damage WHERE t_damage.date between :fromDate and :toDate "
			+ "and t_damage.del_status=1 AND m_item.item_id=t_damage.item_id),0) AS damage_value, coalesce(0) AS gatepass_qty, "
			+ "coalesce(0) AS gatepass_return_qty FROM m_item where m_item.is_used=1 and  m_item.item_id=:itemId"),nativeQuery=true)
