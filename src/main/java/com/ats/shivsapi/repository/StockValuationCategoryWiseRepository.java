...
public interface StockValuationCategoryWiseRepository extends JpaRepository<StockValuationCategoryWise, Integer>{
	...
	@Query(value=("SELECT \r\n" + 
			...
			"            and t_mrn_detail.del_status=1 and m_item.cat_id=m_category.cat_id AND t_mrn_detail.mrn_detail_status = 4),\r\n" + 
			"        0) AS approve_qty,\r\n" + 
			"        coalesce((Select\r\n" + 
			...
			"            and po_detail.po_detail_id=t_mrn_detail.po_detail_id and m_item.cat_id=m_category.cat_id AND t_mrn_detail.mrn_detail_status = 4),\r\n" + 
			...
			"            and item_issue_detail.del_status=1 and m_item.cat_id=m_category.cat_id AND item_issue_detail.status = 2),\r\n" + 
			"        0) AS issue_qty,\r\n" + 
			"        coalesce((Select\r\n" + 
			...
			"            and po_detail.po_detail_id=t_mrn_detail.po_detail_id and m_item.cat_id=m_category.cat_id AND item_issue_detail.status = 2),\r\n" + 
	...
	@Query(value=(" SELECT m_category.cat_id, m_category.cat_desc, coalesce(0) AS opening_stock, coalesce(0) AS op_stock_value, "
			...
			+ "and t_mrn_detail.del_status=1 and po_detail.po_detail_id=t_mrn_detail.po_detail_id and m_item.cat_id=m_category.cat_id and t_mrn_header.mrn_type=:typeId AND t_mrn_detail.mrn_detail_status = 4), 0) AS approved_landing_value , "
			...
			+ "and t_mrn_header.mrn_type=:typeId AND t_mrn_detail.mrn_detail_status = 4), 0) AS approved_qty_value, coalesce((Select SUM(item_issue_detail.item_issue_qty) FROM item_issue_header, "
			...
			+ "and po_detail.po_detail_id=t_mrn_detail.po_detail_id and m_item.cat_id=m_category.cat_id and item_issue_header.item_category=:typeId AND item_issue_detail.status = 2), 0) AS issue_landing_value, "
			...
			+ "and item_issue_detail.mrn_detail_id=t_mrn_detail.mrn_detail_id and po_detail.po_detail_id=t_mrn_detail.po_detail_id and m_item.cat_id=m_category.cat_id and item_issue_header.item_category=:typeId AND item_issue_detail.status = 2), 0) AS issue_qty_value , "
			+ "coalesce(0) AS damage_qty, coalesce(0) AS damage_value FROM m_category where m_category.is_used=1  "),nativeQuery=true)
