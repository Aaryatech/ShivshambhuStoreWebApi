...
public class UserMstController {
	 ...
	 * public @ResponseBody Info saveUniqueUser(@RequestParam("usrMob") String
	 * usrMob) {
	 * 
	 * User res = new User(); Info info = new Info();
	 * 
	 * try { res = userRepo.findByUsrMobAndDelStatus(usrMob, 1); if (res == null) {
	 * 
	 * info.setError(false); info.setMessage("New User");
	 * System.out.println("In If"); } else { info.setError(true);
	 ...
	 * User user = new User();
	 * 
	 * try { user = userRepo.findByUserIdAndDelStatus(userId, 1);
	 ...
	 * Info info = new Info();
	 * 
	 * try { int delete = userRepo.deleteUser(userId);
	 * 
	 * if (delete == 1) { info.setError(false);
	 * info.setMessage("successfully Deleted"); } else { info.setError(true);
	 ...
	 * public @ResponseBody Info updateUserToken(@RequestParam("userId") int
	 * userId, @RequestParam("token") String token) {
	 * 
	 * Info info = new Info();
	 * 
	 * try { int delete = userRepo.updatetoken(userId, token);
	 * 
	 * if (delete == 1) { info.setError(false);
	 * info.setMessage("successfully Updated"); } else { info.setError(true);
	 * info.setMessage(" Deleted to Delete"); }
	 * 
	 * } catch (Exception e) {
	 * 
	 ...
	 * public @ResponseBody Info deleteMultiUser(@RequestParam("userIds")
	 * List<Integer> userIds) {
	 * 
	 * Info info = new Info();
	 * 
	 * try { int delete = userRepo.deleteMultiUser(userIds);
	 * 
	 * if (delete >= 1) { info.setError(false);
	 * info.setMessage("successfully Multiple Deleted"); } else {
	 * info.setError(true); info.setMessage(" Deleted to Delete"); }
	 * 
	 * } catch (Exception e) {
	 * 
	 * e.printStackTrace(); info.setError(true);
	...
	public @ResponseBody LoginResUser loginUser(@RequestParam("usrMob") String usrMob,
			@RequestParam("userPass") String userPass, @RequestParam("deptId") int deptId) {

		LoginResUser loginResponse = new LoginResUser();
		try {

			User mu = userRepo.findByUsrMobAndUserPassAndDelStatusAndDeptId(usrMob, userPass, 1, deptId);
			if (mu == null) {
